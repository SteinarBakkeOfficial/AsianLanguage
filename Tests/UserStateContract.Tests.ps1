...
Assert-Contains -Text $userStateText -ExpectedSubstring "var currentCharacterID: String?" -Message "App user state should persist the explicit current character."
Assert-Contains -Text $userStateText -ExpectedSubstring "var hasCompletedOnboarding: Bool" -Message "App user state should persist the onboarding gate."
Assert-Contains -Text $userStateText -ExpectedSubstring "enum AppearancePreference" -Message "App user state should persist only the required appearance preference."
...
Assert-Contains -Text $userStateText -ExpectedSubstring "legacy keys are decode-only" -Message "App user state should not write legacy keys."
...
Assert-Contains -Text $userStateText -ExpectedSubstring "learnedAt" -Message "Lesson state should retain completion timing when available."
...
Assert-Contains -Text $storeText -ExpectedSubstring "func resetLearningProgress()" -Message "Local user-state store should reset learning only."
Assert-Contains -Text $storeText -ExpectedSubstring "func resetAllPreferences()" -Message "Local user-state store should reset all preferences separately."
