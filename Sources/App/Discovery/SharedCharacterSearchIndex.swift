...
struct SharedCharacterSearchIndex {
    ...
    private func searchableFields(for record: SharedCharacterRecord) -> [String] {
        ...
        fields.append(contentsOf: record.focusCoverage.simplifiedChinese.variants.flatMap { [$0.form] + $0.readings.map(\.value) + $0.notes })
        fields.append(contentsOf: record.focusCoverage.traditionalChinese.variants.flatMap { [$0.form] + $0.readings.map(\.value) + $0.notes })
        fields.append(contentsOf: record.focusCoverage.japanese.variants.flatMap { [$0.form] + $0.readings.map(\.value) + $0.notes })
        fields.append(contentsOf: record.focusCoverage.korean.variants.flatMap { [$0.form] + $0.readings.map(\.value) + $0.notes })
        fields.append(contentsOf: record.focusCoverage.traditionalChinese.taiwanReadings.map(\.value))
        fields.append(contentsOf: record.focusCoverage.traditionalChinese.hongKongReadings.map(\.value))
